...
export default function CancelarModal({
    ...
    return (
        <Box sx={{ p: 1, minWidth: { sm: 400 } }}>
            ...
            <Typography variant="body2" color="text.secondary" mb={3}>{descricao}</Typography>
            <TextField
                label="Motivo do Cancelamento"
                multiline rows={3} fullWidth value={motivo}
                onChange={(e) => { setMotivo(e.target.value); setError(''); }}
                placeholder="Ex: Cliente desistiu..." error={!!error} helperText={error} autoFocus
            />
            <Box display="flex" gap={2} mt={4} justifyContent="flex-end">
                <Button onClick={onCancel} variant="outlined" color="inherit" disabled={loading}>Voltar</Button>
                <Button onClick={handleConfirm} variant="contained" color="error" loading={loading}>Confirmar</Button>
            </Box>
        </Box>
    );
}