...
// Mapeamento de nomes de mês para índice (0-11)
        ...
        >
            {data.map((item) => (
                <Tooltip
                    ...
                    arrow
                    placement="top"
                    title={`Dia ${item.day}: ${formatCurrency(item.total)} (${item.orders} pedidos)`}
                >
                    <Box
                        sx={{
                            width: '20px',
...
// --- Componente KPI: Resumo do Dia ou Período ---
const DailyPeriodSummary = ({
    ...
    const totalPayments =
        Object.values(data.payments).reduce((a, b) => a + b, 0) || 1;
...
export default function VendasModalContent() {
    ...
    // --- BUSCA E PROCESSAMENTO DE DADOS REAIS ---
    useEffect(() => {
        const fetchAndProcessData = async () => {
            setLoading(true);
            try {
                // CORREÇÃO: Buscando da coleção 'pedidos' com status 'entregue'
                const q = query(
                ...
                snapshot.docs.forEach(doc => {
                    ...
                    // CORREÇÃO: Pega data do pagamento ou data de criação como fallback
                    ...
                    // Usa a estrutura nova: pagamento.pagamentos = { 'Dinheiro': 50, ... }
                    ...
                    if (Object.keys(pags).length > 0) {
                        ...
                        // Fallback para pedidos antigos sem array detalhado
                        // Se não tiver info, assume dinheiro ou cartão genérico
                        methodAmounts.cartao = total;
                    }

                    // --- ESTRUTURAÇÃO DOS DADOS (AGREGAÇÃO) ---
                    if (!structure[year]) structure[year] = {};

                    if (!structure[year][monthName]) {
                        structure[year][monthName] = {
                            total: 0,
                            payments: { pix: 0, cartao: 0, dinheiro: 0, voucher: 0 },
                            daily: []
                        };
                    }
                ...
                // Seleciona o ano mais recente se não tiver um selecionado
                const availableYears = Object.keys(structure).map(Number).sort((a, b) => b - a);
                if (availableYears.length > 0 && !structure[anoSelecionado]) {
    ...
    const handleAnoChange = (event: SelectChangeEvent<number>) => {
        ...
        const dataAno = salesData[event.target.value as number];
        if (dataAno && !dataAno[mesSelecionado]) {
            setMesSelecionado(Object.keys(dataAno)[0] || 'Jan');
        }
    };
    const handleMesChange = (event: SelectChangeEvent) => setMesSelecionado(event.target.value);
    const handleSubFiltroDiaChange = (e: any, newVal: string | null) => {
    ...
    const monthlyData = monthNames.map(m => {
        const mData = dataAno[m];
        return {
            label: m,
            total: mData ? mData.total : 0,
            payments: mData ? mData.payments : { pix: 0, cartao: 0, dinheiro: 0, voucher: 0 }
        };
    }).filter(d => d.total > 0);
    ...
    const yearlyData = Object.keys(salesData).map(yKey => {
        ...
        Object.values(yData).forEach((m: MonthData) => {
            total += m.total;
            payments.pix += m.payments.pix;
            payments.cartao += m.payments.cartao;
            payments.dinheiro += m.payments.dinheiro;
            payments.voucher += m.payments.voucher;
        });
    ...
    if (subFiltroDia === 'diaUnico' && diaUnico && !loading) {
        ...
        if (dData) {
            diaUnicoData = dData;
            diaUnicoTitle = `Resumo do Dia: ${d}/${mStr}/${y}`;
        }
    }
    ...
    if (subFiltroDia === 'periodo' && diaInicio && diaFim && !loading) {
        ...
        Object.entries(salesData).forEach(([yKey, months]) => {
            ...
            Object.entries(months as YearData).forEach(([mName, mData]) => {
                ...
                (mData as MonthData).daily.forEach((day: DailyData) => {
                    const current = new Date(year, mIndex, day.day);
                    if (current >= start && current <= end) {
                        found = true;
                        total += day.total;
                        orders += day.orders;
                        payments.pix += day.payments.pix;
                        payments.cartao += day.payments.cartao;
                        payments.dinheiro += day.payments.dinheiro;
                        payments.voucher += day.payments.voucher;
                    }
                });
            });
        });
        ...
        if (found) {
            ...
            periodoTitle = `Período: ${start.toLocaleDateString('pt-BR')} até ${end.toLocaleDateString('pt-BR')}`;
        }
    }
    ...
    return (
        <Stack spacing={3} sx={{ minWidth: { xs: '80vw', sm: '70vw', md: '50vw' } }}>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }}>
                ...
                {tipoVisao === 'Mensal' && (
                    <FormControl size="small" fullWidth>
                        <Typography variant="caption" sx={{ fontWeight: 600, mb: 0.5 }}>Ano</Typography>
                        <Select value={anoSelecionado} onChange={handleAnoChange}>
                            {Object.keys(salesData).map(y => <MenuItem key={y} value={Number(y)}>{y}</MenuItem>)}
                        </Select>
                    </FormControl>
                )}
                ...
                {tipoVisao === 'Diário' && (
                    <FormControl size="small" fullWidth>
                        <Typography variant="caption" sx={{ fontWeight: 600, mb: 0.5 }}>Mês</Typography>
                        <Select value={mesSelecionado} onChange={handleMesChange}>
                            {Object.keys(dataAno).length > 0
                                ? Object.keys(dataAno).map(m => <MenuItem key={m} value={m}>{m}/{anoSelecionado}</MenuItem>)
                                : <MenuItem disabled>Sem dados</MenuItem>
                            }
                        </Select>
                    </FormControl>
                )}
            ...
            {tipoVisao === 'Diário' && (
                <Paper variant="outlined" sx={{ p: 2, bgcolor: 'background.paper' }}>
                    ...
                    {subFiltroDia === 'diaUnico' && (
                        <TextField type="date" label="Selecione o Dia" value={diaUnico} onChange={e => setDiaUnico(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} />
                    )}

                    {subFiltroDia === 'periodo' && (
                        <Stack direction="row" spacing={2}>
                            <TextField type="date" label="Início" value={diaInicio} onChange={e => setDiaInicio(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} />
                            <TextField type="date" label="Fim" value={diaFim} onChange={e => setDiaFim(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} />
                        </Stack>
                    )}
                </Paper>
            )}
            ...
            <Box>
                <Typography variant="h6" gutterBottom>
                    {tipoVisao === 'Mensal' && `Vendas em ${anoSelecionado}`}
                    {tipoVisao === 'Anual' && 'Histórico Anual'}
                ...
                {tipoVisao === 'Diário' && subFiltroDia === 'diaUnico' && (
                    diaUnicoData
                        ...
                        : <Paper variant="outlined" sx={{ p: 3, textAlign: 'center' }}><Typography variant="body1">Nenhum dado encontrado para o dia selecionado.</Typography></Paper>
                )}
                {tipoVisao === 'Diário' && subFiltroDia === 'periodo' && (
                    periodoData
                        ? <DailyPeriodSummary data={periodoData} title={periodoTitle} />
                        : <Paper variant="outlined" sx={{ p: 3, textAlign: 'center' }}><Typography variant="body1">{diaInicio && diaFim ? 'Nenhum dado no período.' : 'Selecione as datas.'}</Typography></Paper>
                )}
            </Box>
        </Stack>
    );
